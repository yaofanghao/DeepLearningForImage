...
if __name__ == "__main__":
    ...
    dir_origin_path = "ROCtest/demo2/"  # 示例，要评估ROC的jpg和png所在目录
    ...
    if class_num == 1:
        ...
        for i in range(0, pr.shape[0]):
            for j in range(0, pr.shape[2]):
                y_pred_num.append(pr[i, class_num, j])
                sum += 1
        ...
        for i in range(0, y_true.shape[0]):
            for j in range(0, y_true.shape[2]):
                y_true_num.append(y_true[i, class_num, j])
                sum_true += 1
    ...
    plt.show()
    plt.savefig(os.path.join(dir_origin_path, 'roc.png'))
