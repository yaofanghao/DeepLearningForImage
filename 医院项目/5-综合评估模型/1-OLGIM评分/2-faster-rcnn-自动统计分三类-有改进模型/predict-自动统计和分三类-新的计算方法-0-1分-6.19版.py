...
if __name__ == "__main__":
    ...
    # 存放全部图片预测结果数据的txt
    f1 = open(os.path.join(os.getcwd(), 'result_predict.txt'), 'w')

    # 存放softmax转换后的预测结果数据的txt
    f_softmax = open(os.path.join(os.getcwd(), 'result_softmax.txt'), 'w')

    # 存放经过新的计算方法得到新分数的txt
    f_new_scores = open(os.path.join(os.getcwd(), 'result_new_scores_0_1.txt'), 'w')
    ...
    f = open(os.path.join(os.getcwd(), 'report_predict.txt'), 'w')
