...
if __name__ == "__main__":
    ...
    if mode == "dir_predict":
        ...
        if confidence_down < 0.92:
            # 创建图片输出的文件夹
            all_save_path_0 = str(dir_save_path) + "_0/"
            all_save_path_1 = str(dir_save_path) + "_1/"
            ...
            if not os.path.exists(all_save_path_1):
                os.makedirs(all_save_path_1)
            ...
            for img_name in img_names:
                ...
                # -----------------------------------------------------------------------------------------------#
                    ...
                    f1.write("对应的置信度分数为：")
                    f1.write("\r")
                    numpy.savetxt(f1,out_scores.numpy())
                    ...
                    if out_scores.numpy().size != 0:
                        ...
                        for i in out_classes.numpy():
                            ...
                            if i == 1:
                                class1 = numpy.append(class1, t[num])
                            if i == 2:
                        ...
                        class1_max = numpy.max(class1)
                        class1_max=round(class1_max, 4)
                    ...
                    # -----------------------------------------------------------------------------------------------#
                        ...
                        if class_max_confidence == 1:
                            num1 += 1
                            r_image.save(os.path.join(all_save_path_1, img_name.replace(".jpg", ".png")),quality=95, subsampling=0)
            ...
            print("识别为0分的比例：")
            print(str((num0 / (num0+num1+num2+num3+none))*100) + '%')
            print("识别为1分的比例：")
            print(str((num1 / (num0+num1+num2+num3+none))*100) + '%')
            print("识别为2分的比例：")
            print(str((num2 / (num0+num1+num2+num3+none))*100) + '%')
            print("识别为3分的比例：")
            print(str((num3 / (num0+num1+num2+num3+none))*100) + '%')
            print("无结果的比例：")
            print(str((none / (num0+num1+num2+num3+none))*100) + '%')
            ...
            f.write("识别为0分：" + str(num0))
            f.write("\r")
            f.write("识别为1分：" + str(num1))
            f.write("\r")
            ...
            f.write("\r")
            f.write("识别为0分的比例：" + str((num0 / (num0+num1+num2+num3+none))*100) + '%')
            f.write("\r")
            f.write("识别为1分的比例：" + str((num1 / (num0+num1+num2+num3+none))*100) + '%')
            f.write("\r")
            f.write("识别为2分的比例：" + str((num2 / (num0+num1+num2+num3+none))*100) + '%')
            f.write("\r")
            f.write("识别为3分的比例：" + str((num3 / (num0+num1+num2+num3+none))*100) + '%')
            f.write("\r")
            f.write("无结果的比例：" + str((none / (num0+num1+num2+num3+none))*100) + '%')
            f.write("\r")
            f.close()
