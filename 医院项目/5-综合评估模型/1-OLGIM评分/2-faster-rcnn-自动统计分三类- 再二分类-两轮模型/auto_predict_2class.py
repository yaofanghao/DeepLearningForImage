...
from frcnn_2class import FRCNN
from tqdm import tqdm

def predict_2class(dir_origin):
    frcnn_2class = FRCNN()

    dir_origin_path = str(dir_origin) + "_img_out_temp/"
    dir_save_path = "img_out"
    all_save_path_2 = str(dir_origin) + "_" + str(dir_save_path) + "_2/"
    all_save_path_3 = str(dir_origin) + "_" + str(dir_save_path) + "_3/"
    all_save_path_none = str(dir_origin) + "_" + str(dir_save_path) + "_none/"
    ...
    for img_name in tqdm(img_names):
        ...
        # 二分类模型预测
        r_image, out_scores, out_classes, _, _, _, _ = frcnn_2class.detect_image(image)
        # print(out_scores)
        # print(out_classes)
        ...
        if (out_scores.size != 0) & (out_scores[0] > 0):
            ...
            if class_max_confidence == 1:
                ...
                r_image.save(os.path.join(all_save_path_3, img_name.replace(".jpg", ".png")),
                             quality=95, subsampling=0)
...
if __name__ == "__main__":
    ...
    # 默认为 test 文件夹
    # 示例：如果要预测文件夹 dir1
    # 则在终端输入： python auto_predict_3class.py dir1
    if len(sys.argv) == 1:
        dir_origin = "test"
    else:  # 手动传入待预测文件夹名称
        dir_origin = sys.argv[1]
    predict_2class(dir_origin)
