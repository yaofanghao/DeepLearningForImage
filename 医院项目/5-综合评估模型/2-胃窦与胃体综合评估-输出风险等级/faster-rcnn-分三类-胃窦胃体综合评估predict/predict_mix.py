...
#   平均值四舍五入规则： [0, 1.5) ->1  [1.5, 2.5) ->2  [2.5,3] ->3
# A和B再进行OLGIM的综合评估，得到结果为 0-1 / 2 / high
# update 2023.4.29
...
if __name__ == "__main__":
    ...
    # image = Image.open("./img/张秋水/胃窦/张秋水 胃窦 (1).jpg")
    # r_image, out_scores, out_classes, top, right, left, bottom = frcnn.detect_image(image)

    if len(sys.argv) == 1:
        print("example: python predict_mix.py img")
        name = "img"  # 输入图片文件夹名称 不带斜杠
    else:  # 手动传入待预测人名名称
        name = sys.argv[1]
    ...
    # f1 = open(os.path.join(base_dir_path, str(name) + '_predict_result.txt'), 'a')
    f1 = open(os.path.join(str("img\\") + str(name) + '_predict_result.txt'), 'a')
    f1.write("【人名：{}】\n".format(str(name)))
    f1.write("【胃窦】\n")
    ...
    for img_name in img_names:
        if img_name.lower().endswith(
            ...
            f1.write((str("/") + str(img_name)).replace(" ", ""))  # 去除图片名中存在的空格
            ...
            if (out_scores.size != 0) & (out_scores[0] > 0):
                ...
                out_scores_max = round(out_scores_max, 6)  # 小数点后4位
                a = out_classes
                ...
                if class_max_confidence == 1:
                    ...
                    r_image.save(os.path.join(all_save_path_2, img_name.replace(".jpg", ".png")),
                                 quality=95, subsampling=0)
                ...
                if class_max_confidence == 2:
                    ...
                    r_image.save(os.path.join(all_save_path_high, img_name.replace(".jpg", ".png")),
                                 quality=95, subsampling=0)
                print("    class_max_confidence:", class_max_confidence)
                f1.write(" {} {}\n".format(str(class_max_confidence), str(out_scores_max)))
            else:  # 没有识别出任何一个框
    ...
    f1.write("胃窦和胃体最终评估结果为：{} \n".format(str(final_result)))
    f1.write("---------------------- \n")
    f1.close()
