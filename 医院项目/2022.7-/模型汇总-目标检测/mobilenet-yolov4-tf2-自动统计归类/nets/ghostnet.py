...
def _squeeze(inputs, hidden_channel, ratio, block_id, sub_block_id):
    ...
    x = Conv2D(_make_divisible(hidden_channel/ratio, 4), (1,1), strides=(1,1), padding='same', kernel_initializer=RandomNormal(stddev=0.02),
            name="blocks."+str(block_id)+"."+str(sub_block_id)+".se.conv_reduce")(x)
    x = Activation('relu')(x)

    x = Conv2D(hidden_channel, (1,1),strides=(1,1), padding='same', kernel_initializer=RandomNormal(stddev=0.02),
...
def _ghost_bottleneck(inputs, output_channel, hidden_channel, kernel, strides, ratio, squeeze, block_id, sub_block_id):
    ...
    if strides > 1:
        ...
        #--------------------------------------------------------------------------#
        x = DepthwiseConv2D(kernel, strides, padding='same', depth_multiplier=1, use_bias=False, depthwise_initializer=RandomNormal(stddev=0.02),
    ...
    if strides == 1 and input_shape[-1] == output_channel:
        ...
        res = DepthwiseConv2D(kernel, strides=strides, padding='same', depth_multiplier=1, use_bias=False, depthwise_initializer=RandomNormal(stddev=0.02),
        ...
        res = Conv2D(output_channel, (1, 1), padding='same', strides=(1, 1), use_bias=False, kernel_initializer=RandomNormal(stddev=0.02),
...
def Ghostnet(inputs):
    ...
    x = Conv2D(16, (3, 3), padding="same", strides=(2, 2), use_bias=False, kernel_initializer=RandomNormal(stddev=0.02), name="conv_stem")(inputs)
