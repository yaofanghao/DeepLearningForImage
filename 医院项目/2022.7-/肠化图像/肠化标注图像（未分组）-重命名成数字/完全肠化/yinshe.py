...
import sys
def renameall():
	fileList = os.listdir(r"E:\PycharmProjects\TensorFlow\医院项目\2022.7-\肠化图像\肠化标注图像（未分组）\完全肠化\xml1")		#待修改文件夹
	saveBasePath = r"E:\PycharmProjects\TensorFlow\医院项目\2022.7-\肠化图像\肠化标注图像（未分组）\完全肠化\xml1"
	print("修改前："+str(fileList))		#输出文件夹中包含的文件
	print("---------------------------------------------------")
	T = str(fileList)
	print(T)
	currentpath = os.getcwd()		#得到进程当前工作目录
	os.chdir(r"E:\PycharmProjects\TensorFlow\医院项目\2022.7-\肠化图像\肠化标注图像（未分组）\完全肠化\xml1")		#将当前工作目录修改为待修改文件夹的位置
	num=1
	#名称变量
	##顺序
