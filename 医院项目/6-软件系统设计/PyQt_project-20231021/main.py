...
from widgets.ChanghuaWidget import Ui_Form as Changhua_ui
from widgets.OLGIMWidget import Ui_Form as OLGIM_ui
...
class InitUi(QtWidgets.QMainWindow, Init_Ui):
    ...
    def __init__(self):
        ...
        self.setupUi(self)
        self.pushButton1.clicked.connect(self.goChanghua)  # 按下按钮1去肠化模块
...
class ChanghuaUi(QtWidgets.QMainWindow, Changhua_ui):
    ...
    def predictJpg(self):
        ...
        for image_num in self.file_paths:
            ...
            if not os.path.exists(os.path.join('./img_out_Changhua/', img_name)):
                ...
                # 关键部分！返回新的画框图和新的置信度，并实现了分类至CIM\IIM等：
...
class Controller:
    ...
    def showInit(self):
        self.initUi.switch_changhua.connect(self.showChanghua)
