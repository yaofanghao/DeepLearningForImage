...
def classifier_smooth_l1(num_classes, sigma=1.0):
    ...
    def class_loss_regr_fixed_num(y_true, y_pred):
        ...
        # regression_loss = 4 * K.sum(y_true[:, :, :
        #                                          4*num_classes] *
        #                             (x_bool * (0.5 * regression_diff * regression_diff) +
        #                              (1 - x_bool) * (regression_diff - 0.5))) / \
...
class ProposalTargetCreator(object):
    ...
    def calc_iou(self, R, all_boxes):
        ...
        y_class_regr_coords[np.arange(np.shape(gt_roi_loc)[0])[:pos_roi_per_this_image], np.array(gt_roi_label[:pos_roi_per_this_image], np.int32)] = gt_roi_loc[:pos_roi_per_this_image]
