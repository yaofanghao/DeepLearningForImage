...
output_dir_sobel = r"E:\MyGithub\cannytest\2-sobel_horizontal\\"
output_dir_suanfa2 = r"E:\MyGithub\cannytest\3-suanfa2\\"

if not os.path.exists(output_dir_sobel):
    os.makedirs(output_dir_sobel)
...
for i in img_list:
    ...
    save_path_sobel = output_dir_sobel + str((ind + 1)) + '.jpg'  # 保存至另一文件夹
    cv2.imwrite(save_path_sobel, sobel_horizontal)
    cv2.imwrite("sobel.jpg", sobel_horizontal)
    # canny边缘检测
    ...
    # # 标记轮廓
    cv2.drawContours(sobel_horizontal, contours, -1, (255, 0, 255), 3)
