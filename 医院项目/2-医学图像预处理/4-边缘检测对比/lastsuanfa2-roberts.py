...
output_dir_roberts = r"E:\MyGithub\cannytest\2-roberts\\"
output_dir_suanfa2 = r"E:\MyGithub\cannytest\3-suanfa2\\"

if not os.path.exists(output_dir_roberts):
    os.makedirs(output_dir_roberts)
...
for i in img_list:
    ...
    # Roberts边缘检测
    # sobel_horizontal = cv2.(img, cv2.CV_64F, 1, 0, ksize=5)
    # Roberts算子
    ...
    save_path_roberts = output_dir_roberts + str((ind + 1)) + '.jpg'  # 保存至另一文件夹
    cv2.imwrite(save_path_roberts, Roberts)
    ...
    # # 标记轮廓
    cv2.drawContours(Roberts, contours, -1, (255, 0, 255), 3)
