import ImgParser as ImgP
import LabelParser as LabelP
import os
import cv2
import time
...
class DataAugmentVOC:
    ...
    def rotate(self,angle=5, scale=1.):
        ...
        for img, xml in tqdm(zip(self.ImgPathlist, self.XMLPathlist)):
            ...
            cv2.imwrite(self.save_imgpath + "\\rotate_" + str(count) + "_" + str(angle) + ".jpg",
                        rot_img,
                        [int(cv2.IMWRITE_JPEG_QUALITY), 95]
                        )
            ...
            self.labP.rotate_Object(rot_mat,
                                    ...
                                    h=rot_img.shape[1],
                                    c=rot_img.shape[2],
                                    save_path = self.save_xmlpath + "\\rotate_" + str(count) +
                                                "_" + str(angle) + ".xml",
                                    folder_name='VOC'
                                    )
        end = time.time()
...
if __name__ == '__main__':
    start = time.time()
    V = DataAugmentVOC(rootpath=r'.\TestData')
    # V.addNoise()
    # V.changeLight()
    V.rotate(angle=60)
    V.rotate(angle=120)
