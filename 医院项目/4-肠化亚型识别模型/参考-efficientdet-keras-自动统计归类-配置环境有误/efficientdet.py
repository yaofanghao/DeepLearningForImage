...
class Efficientdet(object):
    ...
    def generate(self):
        ...
        #   载入模型与权值
        #-------------------------------#
        self.efficientdet = efficientdet([self.input_shape[0], self.input_shape[1], 3], self.phi, self.num_classes)
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        preds       = self.efficientdet.predict(image_data)
        #-----------------------------------------------------------#
        #   将预测结果进行解码
        #-----------------------------------------------------------#
        ...
        if top_boxes.numpy().size == 0:
            top = 0
            right = 0
            left = 0
            bottom = 0

        return image, top_boxes, top_label, top, right, left, bottom  # 和原版相比，添加了 out_scores, out_classes
