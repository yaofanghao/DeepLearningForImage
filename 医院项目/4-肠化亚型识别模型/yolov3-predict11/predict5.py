...
####测试混合
            all_save_path_allNEOandNONNE000 =  str(dir_save_path) + "_all_NEO_0/"
            all_save_path_allNEOandNONNEO01 = str(dir_save_path) + "_all_NEO_1/"
    ...
    ##测试混合的情况下
            if not os.path.exists(all_save_path_allNEOandNONNE000):
                os.makedirs(all_save_path_allNEOandNONNE000)
            if not os.path.exists(all_save_path_allNEOandNONNEO01):
                os.makedirs(all_save_path_allNEOandNONNEO01)
            # 统计识别和未识别图片个数
            num_NEO = 0
            num_NONNEO = 0
            fail = 0
                ...
                if img_name.lower().endswith(
                    ...
                    if out_scores.numpy().size != 0:
                        ...
                        if flag5:  # 如果同时有0和1，进入这个算法
                            ...
                            if(class1==0 ):
                                #若是大于0.5
                                 if(out_scores_max>0.5):
                                   r_image.save(os.path.join(all_save_path_allNEOandNONNE000, img_name.replace(".jpg", ".png")),
                                         quality=95, subsampling=0)

                                 else:
                                     r_image.save(os.path.join(all_save_path_allNEO,
                                                               img_name.replace(".jpg", ".png")),
                                                  quality=95, subsampling=0)

                            if(class1 == 1 ):
                                if(out_scores_max > 0.5):
                                    r_image.save(os.path.join(all_save_path_allNEOandNONNEO01, img_name.replace(".jpg", ".png")),
                                    quality=95, subsampling=0)

                                else:
                                    r_image.save(os.path.join(all_save_path_allNONNEO,
                                                              img_name.replace(".jpg", ".png")),
                                                 quality=95, subsampling=0)
                            # else:
                            #     r_image.save(
                            #         os.path.join(dir_save_path_fail_output, img_name.replace(".jpg", ".png")),
                            #         quality=95, subsampling=0)
