...
if __name__ == '__main__':
    if mode == 0:
        ...
        # hsv 阈值处理
        mask, hsv = hsv_to_mask(img, 0,20, 80,255, 200,230)
        ...
        for i in range(len(contours)):
            ...
            if (cv2.contourArea(contours[i]) > 0.002 * img_area) \
                    & (roundIndex > 0.2) \
                    & (cntLen[i] > 400):  # 只有轮廓大于某一面积，圆形度小于某一值 才会识别出来
    ...
    if mode == 1:
        ...
        while True:
            ...
            # hsv 分割
            mask, hsv = hsv_to_mask(img, 0, 50, 100, 255, 200, 230)
            # 开运算
            mask = imgopen(mask, 5)
            # 融合
            output = cv2.bitwise_and(img, hsv, mask=mask)
            ...
            for i in range(len(contours)):
                ...
                roundIndex = round(roundIndex, 3)
                if (cv2.contourArea(contours[i]) > 0.002 * img_area)\
                        & (cv2.contourArea(contours[i]) < 0.05 * img_area) \
                        & (roundIndex > 0.15) \
                        & (roundIndex < 0.5) \
                        & (cntLen[i] > 300)\
                        & (cntLen[i] < 800):  # 只有轮廓大于某一面积，圆形度小于某一值 才会识别出来
