...
int main(int argc, char **argv) {
    ...
    config.set_model_from_file(model_path); // NaiveBuffer 格式模型路径
    config.set_threads(CPU_THREAD_NUM); // 工作线程数
    config.set_power_mode(CPU_POWER_MODE); // CPU 运行模式
    ...
    {
        LOG(INFO) << "Go Predict for Camera.";
