...
def onnx_predict(onnx=None, img_name=None,
                 ...
                 file_name=None, location=None, people=None,
                 pipe_number=None, pici_number=None, comment=None,
    ...
    if out_scores.size != 0:
        ...
        for i in range(out_classes.shape[0]):
            if out_classes[i] > 0:
                ...
                insert_detect_result = insert_detect_result + str(_name_classes_gbk[temp]) + str(
                    np.round(out_scores[i], 4)) + ", "
        ...
        #           pipe_number, pici_number, img_name, insert_detect_result,
        #           comment)
        ...
        #              .format(img_name, insert_detect_result, table_name))
...
def predict_main(onnx=None, mode=None,
    ...
    if mode == 0:
        try:
            ...
            raise Exception(e)
        dir_save_path = 'image_save/' + str(file_rootname) + "_img_out/"
        ...
        onnx_predict(onnx=onnx, img_name=filename,
                     ...
                     pipe_number=pipe_number, pici_number=pici_number, comment=comment,
    ...
    if mode == 1:
        try:
            ...
            raise Exception(e)
        output_dir = 'video_save/' + str(file_rootname) + '_img/'    # 保存图片文件夹路径
        ...
        dir_save_path = 'video_save/' + str(file_rootname) + "_img_out/"
        if not os.path.exists(dir_save_path):
            os.makedirs(dir_save_path)
...
if __name__ == "__main__":
    ...
    #   输入配置参数
    _mode, _timeF, _filename, _location, _people, _pipe_number, _pici_number, _comment = load_arg()
    ...
    predict_main(onnx=yolo, mode=int(_mode),
                 ...
                 location=_location, people=_people,
                 pipe_number=_pipe_number, pici_number = _pici_number,comment=_comment,
