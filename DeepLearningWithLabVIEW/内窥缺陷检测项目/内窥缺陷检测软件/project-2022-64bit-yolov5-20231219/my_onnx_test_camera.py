...
# 格式化输出年月日时分秒
formatted_all = current_time.strftime("%Y年%m月%d日%H:%M:%S")
# 生成带有年月日的表名
new_table_name = f"{formatted_date}"
...
table_name = 'result'
if not os.path.exists('camera_save/img'):
    os.makedirs('camera_save/img')
if not os.path.exists('camera_save/img_out'):
    os.makedirs('camera_save/img_out')
...
def onnx_predict(img_name, onnx_,
    ...
    image_full_path = os.path.join(output_path, str(img_name))
...
def save_to_database(output_path=None):
    ...
    for i in range(0, len(lines), 3):  # 按照每隔一行的间隔遍历列表
        ...
        insert_data_sql = '''
            INSERT INTO {} VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        '''.format(table_name)
        values = (formatted_all, _location, _people,
