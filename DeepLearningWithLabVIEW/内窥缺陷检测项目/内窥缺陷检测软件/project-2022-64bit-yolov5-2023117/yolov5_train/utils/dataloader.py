...
class YoloDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if len(box) != 0:
            #---------------------------------------------------#
            #   对真实框进行归一化，调整到0-1之间 image, box      = self.get_random_data(self.annotation_lines[index], self.input_shape, random = self.train)
            #---------------------------------------------------#
    ...
    def get_random_data(self, annotation_line, input_shape, jitter=.3, hue=.1, sat=0.7, val=0.4, random=True):
        ...
        if np.random.rand()<0.7:
            ...
            image=enhancer.enhance((np.random.uniform(1.2,2)))
        # 随机进行饱和度增强减弱
        if np.random.rand()<0.7:
            enhancer=ImageEnhance.Color(image)
            image=enhancer.enhance((np.random.uniform(0.5,1.8)))
        #------------------------------------------#
        #   将图像多余的部分加上灰条
        #------------------------------------------#
