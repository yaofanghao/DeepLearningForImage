...
def get_train_step_fn(strategy):
    @tf.function
    def train_step(images, labels, net, optimizer, loss, aux_branch, metrics):
...
def fit_one_epoch(net, loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, aux_branch, metrics, save_period, save_dir, strategy):
